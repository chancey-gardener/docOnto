...
TYPE_MATCH = r"<class '(?:tppProcessor|__main__)\.(\w*)'>"
...
def new_unit(name, pattern, subunit):
	if subunit:
		bclass = IndexUnit
	else:
		bclass = PassageUnit
	
	class newname(bclass):
		SubUnit = subunit
		pattern = pattern
		__name__ = name
...
class TextUnit:
	...
	def __init__(self, header, text, SubUnit):
		...
		if self.SubUnit:
			...
			subunit_name = self.SubUnit.__name__.lower()
			if subunits_or_not:
...
class IndexUnit(TextUnit):
	...
	def _getSubUnits(self, raw):
		...
		while not smatches:
			if self.SubUnit.SubUnit is not None:
				...
				# this feels sketchy
				# print("WARNING: We've got a blank {}".format(self._getTypeName()))
	...
	def getFlatText(self, out=iter([])):
		su_attr = getattr(
						self, self.SubUnit.__name__.lower())
		for subunit_key in su_attr:
			su_val = su_attr[subunit_key]
			if hasattr(su_val, "text"):
				out = chain(out, su_val.text.sents)
			else:
				out = chain(out, su_val.getFlatText(out=out))
		return out
